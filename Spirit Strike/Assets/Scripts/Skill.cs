...
public class Skill : MonoBehaviour
{
    ...
    public struct SkillStat
    {
        ...
        public float _duration;            // 스킬 효과 지속시간, 0이면 단발성, 1 이상이면 지속 스킬
        public float _castRange;           // 스킬 사용 거리
        public float _projectileSpeed;     // 스킬 투사체 이동 속도
        public int _healAmount;            // 치유 스킬의 HP 회복량

        public SkillStat(bool ready, float coolTime, int damage, int target, float duration, float range, float speed, int heal)
        {
            _isReady = ready;
            _coolDown = coolTime;
            _damage = damage;
